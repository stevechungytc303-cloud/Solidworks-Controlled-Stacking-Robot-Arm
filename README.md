...
Built inverse kinematics system in C++ that converts Cartesian XYZ coordinates into robot joint‑angle instructions, using geometric relationships to determine the required joint positions. Also implemented a linear motion control system that enables the end effector to move in straight or diagonal paths by dynamically adjusting motor speeds.
